...
* override.cfg or credentials.cfg
	* Contains Github and Slack API keys. The Slack key must be a legacy token. 
      Generated by `set_config.py`
* projects.json
	* Outlines which data sources to pull from
      Updated by `get_projects.py`
* identities.yaml
	* Maps individual people to usernames, emails, and organizations
      Generated by `create_identities.py` from `people.yaml`
...
You can run the docker container locally for debugging, and if you have the credentials, connect to the same Elasticsearch and Maria instances as Heroku.

1. **Stop the container running on Heroku.**
1. Run this command to run the generated `test_dockerfile`
...
Alternatively, if you want to run the container locally, but use local files to test their effects before pushing to github, you can:
...
You can run a full grimoirelab instance locally.

1. Copy `devstack/override.cfg.sample` file into `devstack/override.cfg`.
...
##### Why do I have to provide a GitHub API token?

Short answer: you don't.

Long answer: Unless you do, the Github API rate limiting will seriously hamper your ability to fetch all the data used on the Leaderboard.

If you're concerned about GitHub access by this tool, use the token for a GitHub user who does not have access to any sensitive repos.
